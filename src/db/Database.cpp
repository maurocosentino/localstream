...
int Database::insertArtist(const std::string& name) {
  ...
  SQLite::Statement query(db_, "SELECT id FROM artists WHERE name = ?");
  query.bind(1, name);
  ...
  SQLite::Statement insert(db_, "INSERT INTO artists (name) VALUES (?)");
  insert.bind(1, name);
  insert.exec();
...
int Database::insertAlbum(const std::string& title, int artist_id, int year) {
  SQLite::Statement query(
      db_, "SELECT id FROM albums WHERE title = ? AND artist_id = ?");
  query.bind(1, title);
  ...
  SQLite::Statement insert(
      db_, "INSERT INTO albums (title, artist_id, year) VALUES (?, ?, ?)");
  insert.bind(1, title);
...
int Database::insertTrack(const Track& track, bool& inserted) {
  SQLite::Statement query(db_, "SELECT id FROM tracks WHERE file_path = ?");
...
std::vector<Artist> Database::getArtists() {
  ...
  SQLite::Statement query(db_, "SELECT id, name FROM artists ORDER BY name");
...
std::optional<Artist> Database::getArtistById(int artist_id) {
  SQLite::Statement q(db_, "SELECT id, name FROM artists WHERE id = ?");
  q.bind(1, artist_id);
...
std::vector<Album> Database::getAlbums(int artist_id) {
  ...
  SQLite::Statement query(db_,
                          "SELECT id, title, artist_id, year FROM albums WHERE "
                          "artist_id = ? ORDER BY year");
...
std::optional<Album> Database::getAlbumById(int album_id) {
  SQLite::Statement q(
      db_, "SELECT id, title, artist_id, year FROM albums WHERE id = ?");
  q.bind(1, album_id);
...
std::vector<Album> Database::getAlbumList(const std::string& type, int size,
                                          int offset) {
  std::string order;
...
std::string Database::getTrackPath(int track_id) {
  SQLite::Statement query(db_, "SELECT file_path FROM tracks WHERE id = ?");
...
std::string Database::getAnyTrackPath(int album_id) {
  SQLite::Statement q(
      db_, "SELECT file_path FROM tracks WHERE album_id = ? LIMIT 1");
  q.bind(1, album_id);
    ...
    if (f.isValid() && f.tag()) {
      ...
      if (items.contains("covr")) {
        ...
        if (!covers.isEmpty()) {
          ...
          cd.data.assign(reinterpret_cast<const unsigned char*>(data.data()),
                         reinterpret_cast<const unsigned char*>(data.data()) +
                             data.size());
          cd.mime_type = (cover.format() == MP4::CoverArt::PNG) ? "image/png"
                                                                : "image/jpeg";
          return cd;
        }
      }
...
std::optional<localstream::CoverData> Database::getAlbumCover(int album_id) {
  ...
  {
    SQLite::Statement q(
        db_, "SELECT cover_blob, cover_mime FROM albums WHERE id = ?");
    q.bind(1, album_id);
    if (q.executeStep()) {
      ...
      if (!col_blob.isNull() && col_blob.getBytes() > 0) {
        ...
        cd.data.assign(static_cast<const unsigned char*>(ptr),
                       static_cast<const unsigned char*>(ptr) + sz);
        cd.mime_type = col_mime.isNull() ? "image/jpeg" : col_mime.getString();
        return cd;
      }
    }
  }
  ...
  try {
    SQLite::Statement upd(
        db_, "UPDATE albums SET cover_blob = ?, cover_mime = ? WHERE id = ?");
...
int Database::createPlaylist(const std::string& name) {
  SQLite::Statement insert(db_, "INSERT INTO playlists (name) VALUES (?)");
  insert.bind(1, name);
  insert.exec();
  return static_cast<int>(db_.getLastInsertRowid());
}
...
bool Database::deletePlaylist(int playlist_id) {
  SQLite::Statement stmt(db_, "DELETE FROM playlists WHERE id = ?");
...
bool Database::addTrackToPlaylist(int playlist_id, int track_id) {
  ...
  SQLite::Statement pos(db_,
                        ...
                        "playlist_tracks WHERE playlist_id = ?");
