...
bool Transcoder::needsTranscode(const std::string& src_format,
                                const std::string& dst_format,
                                int src_bitrate_kbps, int max_bitrate_kbps) {
...
bool Transcoder::transcode(
  ...
  std::string cmd =
      ...
      " -vn"
      " -c:a " +
      codec + " " + bitrate_arg + " -f " + fmt + " pipe:1 2>/dev/null";
  ...
  if (exit_code != 0) {
    LOG_WARN("Transcoder",
             "ffmpeg terminó con código: " + std::to_string(exit_code));
  }
